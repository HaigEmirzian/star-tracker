...
export default function CapabilityTranslator() {
  ...
  // Defaults to the confirmed per-satellite GPU count for AI1, Starmind's
  // first version (see lib/data/starmindStatic.ts), so the calculator opens
  // on a real number instead of an arbitrary "1".
  const [quantity, setQuantity] = useState(starmind.perSatelliteHardware.rubinGpuCount.value);
  ...
  const totals = computeTotals({ tdpWatts: gpu.tdpWatts, flopsByPrecision: gpu.flops, quantity });
  ...
  const computeComparisons =
    totalFlops > 0 ? computeRelatableComparisons(totalFlops, comparisonFactors) : [];
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-left backdrop-blur-sm">
      ...
      <div className="mb-5">
        ...
        <div className="mt-1 text-xs text-white/30">
          Slider uses a log scale — spans 1 to {QUANTITY_BOUNDS.max.toLocaleString()} GPUs. Defaults
          to {starmind.perSatelliteHardware.rubinGpuCount.value}, the confirmed per-satellite GPU
          count for AI1.
        </div>
      </div>
      ...
      {computeComparisons.length > 0 && (
        <div className="mt-4">
          <div className="mb-2 text-xs uppercase tracking-wide text-white/40">
            Compute comparisons
          </div>
          <RelatableComparisonList comparisons={computeComparisons} />
        </div>
      )}
    </div>
  );
